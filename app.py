...
# ─────────────────────────────────────────────
    ...
    # ── World Map ────────────────────────────────────────
    # Build per-location aggregated data
    loc_coords = {
        "Spain":      {"lat": 40.4637, "lon": -3.7492, "region": "Europe", "country": "Europe"},
        "Germany":    {"lat": 51.1657, "lon": 10.4515, "region": "Europe", "country": "Europe"},
        "Mumbai":     {"lat": 19.0760, "lon": 72.8777, "region": "Asia",   "country": "India"},
        "Singapore":  {"lat": 1.3521,  "lon": 103.8198, "region": "Asia",   "country": "Singapore"},
        "Bangkok":    {"lat": 13.7563, "lon": 100.5018, "region": "Asia",   "country": "Thailand"},
    }
    # Which products each location represents
    loc_products = {
        "Spain":      ["Care - Aqua Warranty"],
        "Germany":    ["Care - Aqua Warranty"],
        "Mumbai":     ["Income Protection Insurance"],
        "Singapore":  ["Income Protection Insurance", "Electric Vehicles / Auto"],
        "Bangkok":    ["Electric Vehicles / Auto"],
    }

    # Pre-aggregate data by country
    ...
    map_lats, map_lons, map_texts, map_hovers, map_sizes, map_colors = [], [], [], [], [], []
    for loc, meta in loc_coords.items():
        cdata = country_agg[country_agg["Country"] == meta["country"]]
        contracts = int(cdata["Contracts"].sum()) if len(cdata) > 0 else 0
        revenue = float(cdata["Revenue"].sum()) if len(cdata) > 0 else 0
        paid = float(cdata["Paid"].sum()) if len(cdata) > 0 else 0
        outstanding = float(cdata["Outstanding"].sum()) if len(cdata) > 0 else 0
        prods = loc_products[loc]
        prod_labels = " + ".join([p.split(" ")[0] for p in prods])  # short labels
        map_lats.append(meta["lat"])
        map_lons.append(meta["lon"])
        map_texts.append(f"<b>{loc}</b>")
        map_hovers.append(
            f"<b>{loc}</b><br>"
            f"Products: {prod_labels}<br>"
            f"Contracts: {contracts:,}<br>"
            f"Revenue: {fmt(convert(revenue, cur), cur)}<br>"
            f"Paid: {fmt(convert(paid, cur), cur)}<br>"
            f"Outstanding: {fmt(convert(outstanding, cur), cur)}"
        )
        map_sizes.append(max(12, min(40, contracts / 5)))
        # Color by primary product
        primary_prod = prods[0]
        map_colors.append(PRODUCT_COLORS.get(primary_prod, PRODUCT_DEFAULT_COLOR))

    fig_map = go.Figure()
    fig_map.add_trace(go.Scattergeo(
        lat=map_lats, lon=map_lons,
        text=map_texts, hovertext=map_hovers,
        hoverinfo="text",
        mode="markers+text",
        textposition="top center",
        textfont=dict(size=12, color=T["heading_color"], family="Inter, sans-serif"),
        marker=dict(
            size=map_sizes,
            color=map_colors,
    ...
    fig_map.update_geos(
        ...
        lonaxis_range=[60, 145],
        lonaxis_showgrid=False,
    )
    fig_map.update_layout(
        height=420,
        margin=dict(l=0, r=0, t=30, b=0),
        title=dict(text="Global Hotspots", font=dict(size=16, color=T["heading_color"])),
        paper_bgcolor="rgba(0,0,0,0)",
    ...
    # Location detail selector
    loc_names = list(loc_coords.keys())
    selected_loc = st.selectbox("🔍 Select a hotspot for details", loc_names, key="map_loc_sel")
    if selected_loc:
        meta = loc_coords[selected_loc]
        prods = loc_products[selected_loc]
        loc_df = df_filtered[
            (df_filtered["Country"] == meta["country"]) &
            (df_filtered["Product Label"].isin(prods))
        ]
        st.subheader(f"📍 {selected_loc}")
        mc1, mc2, mc3, mc4 = st.columns(4)
        mc1.metric("Contracts", f"{len(loc_df):,}")
        mc2.metric("Revenue", fmt(convert(loc_df["Annual Premium"].sum(), cur), cur))
        mc3.metric("Paid", fmt(convert(loc_df["Paid"].sum(), cur), cur))
        mc4.metric("Outstanding", fmt(convert(loc_df["Outstanding"].sum(), cur), cur))

        # Mini breakdown
        if len(loc_df) > 0:
            loc_by_prod = loc_df.groupby("Product Label").agg(
                Contracts=("Contract ID", "count"), Revenue=("Annual Premium", "sum"),
            ).reset_index()
            loc_by_prod.rename(columns={"Product Label": "Product"}, inplace=True)
            loc_by_prod["Revenue"] = loc_by_prod["Revenue"].apply(lambda v: convert(v, cur))
            st.plotly_chart(
                bar_chart(loc_by_prod, "Product", "Contracts", f"Contracts — {selected_loc}", color_map=PRODUCT_COLORS),
